...
| 항목 | 구현 내용 |
| --- | --- |
| 적 스폰 | `PlatformManager`가 발판을 생성할 때 일정 확률로 `Enemy`를 함께 생성합니다. 초반 안전 구간에서는 적을 생성하지 않습니다. |
| 적 점프 | `Enemy.updateWithCollision()`에서 발판 위치를 확인하고, 발판 위에서 주기적으로 점프합니다. |
| 대시 타격 | 플레이어가 대시 중 적과 충돌하면 `enemy.die()`가 호출되고, 적은 오른쪽 위로 날아가며 회전합니다. |
| 밟기 판정 | 플레이어가 낙하 중이고 적의 머리 근처에 닿으면 `player.bounce()`로 반동 점프를 합니다. |
| 사망 판정 | 대시도 아니고 밟기도 아닌 일반 충돌은 `GAME_OVER`로 전환합니다. |
| 디버그 | 적과 플레이어의 실제 충돌 범위를 빨간색 Hitbox로 표시하여 판정을 확인합니다. |
...
| **6주차** | **05/10 ~ 05/16** | 16 | MainScene, 플레이어, 대시, 스크롤, Dash Trail, PlatformManager, 적, 점수 UI, 2차 발표 README |
...
| 구분 | 내용 |
| --- | --- |
| class 구성 정보 | Android Path를 이용한 리본형 대시 잔상 |
...
가장 어려웠던 부분은 **충돌 처리**입니다. 현재는 플레이어와 적의 `RectF`가 겹치는지 확인한 뒤, 플레이어의 낙하 속도와 위치를 이용해 밟기, 대시 타격, 일반 충돌을 구분합니다.

하지만 실제 게임에서는 한 프레임 사이에 위치가 많이 변할 수 있고, 적의 머리 위를 밟은 것인지 옆면에 닿은 것인지 애매한 경우가 생깁니다. 그래서 현재는 빨간색 Hitbox를 표시하면서 판정 범위를 확인하고 있습니다. 이 부분은 구현하면서 가장 시간이 많이 걸렸고, 이후에도 계속 다듬어야 할 부분입니다.
